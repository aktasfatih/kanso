...
#                               but the rest of the stack still comes up)
...
install_optional_app() {
	...
	if docker exec -u www-data kanso-dev php occ app:list | grep -q "^  - $app: $version"; then
		...
		curl -fsSL "$url" -o "/tmp/$app.tar.gz" || {
			echo "Could not download $app from $url." >&2
			echo "Re-run with KANSO_SKIP_OPTIONAL_APPS=1 to boot without it (the two e2e specs above will then fail)." >&2
			exit 1
		}
...
install_optional_app deck "$DECK_VERSION" "$DECK_URL"
install_optional_app contacts "$CONTACTS_VERSION" "$CONTACTS_URL"

...
# 2. Every migration must be recorded as applied: the oc_migrations table must
#    hold one row per migration file, which proves the migrator ran end-to-end
#    (not just that app:enable returned 0). The assertion itself lives in
#    assert-migrations.sh — dev/setup.sh runs the very same one before it reports
#    a boot healthy, and the per-driver counting has no business existing twice.
#    It reads the same KANSO_DB / KANSO_CONTAINER / KANSO_DB_CONTAINER env vars
#    with the same defaults, so it targets whatever stack this run targets, and
#    `set -e` turns its failure into ours.
./assert-migrations.sh
...
case "$KANSO_DB" in
	postgres)      COUNT="$(count_tables_postgres)"; HAS=has_table_postgres ;;
	mysql|mariadb) COUNT="$(count_tables_mysql)";    HAS=has_table_mysql ;;
	sqlite)        COUNT="$(count_tables_sqlite)";   HAS=has_table_sqlite ;;

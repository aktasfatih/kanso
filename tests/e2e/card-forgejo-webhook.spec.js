...
test.describe('Forgejo webhook ingest', () => {
	...
	test('a delivery that matches nothing says why', async () => {
		...
		expect(res.body.reason).toBe('no_card_match')
	})
...
test.describe('Forgejo webhook issue intake', () => {
	...
	test('opened issue creates a linked card, redelivery does not duplicate', async () => {
		...
		// Redelivery: the issue is now LINKED, so this takes the ordinary linked-issue
		// path rather than intake - `opened` maps to no role, hence unknown_action.
		// (The intake_duplicate guard is for a card the alive-link lookup misses,
		// e.g. an archived one - covered by the GitHub suite.) Either way, no
		// second card is created, which is what this test is really about.

...
- **Changelog is part of the change, not an afterthought.** Any user-facing change (feature, bugfix, behavior/UX change) MUST add a line under `## [Unreleased]` in `CHANGELOG.md` in the same PR — Keep a Changelog format, grouped under `### Added` / `### Changed` / `### Fixed`. Pure-internal changes (tests, refactors, tooling) may skip it. At release time the `[Unreleased]` block is renamed to the new version; an empty `[Unreleased]` at release means entries were forgotten. This is easy to miss when work is split across per-card workers — treat "did I touch the changelog?" as a gate in the same breath as psalm/cs.
- **Version bumps are release-only.** Do NOT bump `appinfo/info.xml` `<version>` in a feature PR — the version is bumped solely in the release commit. Migrations may land without a bump; they run on fresh installs/CI immediately and on existing installs at the next release's version bump. (Don't confuse a migration's own `VersionNNNN` file number with the app version.)
